<script lang="ts">
    ...
    import { ArrowRight, BookOpen, GraduationCap } from "lucide-svelte";
    ...
    <main class="max-w-2xl w-full text-center space-y-12">
      ...
      <Card.Root class="border-none shadow-2xl bg-white/90 backdrop-blur-md">
        <Card.Header class="space-y-1 pt-8">
          <Card.Title class="text-3xl font-bold tracking-tight">Ready to level up?</Card.Title>
          <Card.Description class="text-base text-slate-500">
            Access your Vocabulary Hub and start your first Practice Arena session.
          </Card.Description>
        </Card.Header>
        
        <Card.Content class="flex justify-center pb-10">
          <Button 
            variant="default" 
            size="lg" 
